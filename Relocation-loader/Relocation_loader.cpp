...
int main(){
    ...
    a = stoi(s,nullptr,16);         //starting address in hexa
    ...
    while(x!="E"){
        if(x=="H"){
            ...
            input >> x >> x;
            string b = bitset<12>(stoi(x, nullptr, 16)).to_string(); //hexa -> binary -> stored as string
            input >> x;
            int i = 0;
            while(!(x=="T" || x=="E")){
                if(b[i]=='1'){
                    int direct = stoi(x.substr(x.length()-4), nullptr, 16) + a; // address after 14 + a
                    out << hex << address << " " << x.substr(0,2) << direct << endl; // out 14 + direct
                }
                else{
